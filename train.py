...
with_att = False #決定是否使用attention mechanism ,選擇所用的modelel
    ...
    def train(self):
        for fold, (train_idx,val_idx) in enumerate(self.splits.split(np.arange(len(self.dataset)))):
            ...
            for epoch in range(self.start,self.epochs):
                ...
                torch.save( self.model.serialize(self.model,self.optimizer,epoch,self.history_train_loss,self.history_val_loss)\
...
def main():
    ...
    language = 'zh'
    if language == 'Eng':
        vocab_path = '/homes/GPU1/nlp_punctuation/data/vocabulary'
        punc_path = '/homes/GPU1/nlp_punctuation/data/punc_vocab'
    ...
    '''
    if with_att:
        encoder = Encoder(input_size,embedding_dim,hidden_dim,4,1,0.,pretrained)
        decoder = Decoder(hidden_dim,4,1,0.0)
        model = Seq2Seq(encoder,decoder,hidden_dim)
        save_path = '/home/nlp_punc/punc_TBRNN/modelfinal_attn.pth.tar'
    else:
        save_path = '/home/nlp_punc/punc_TBRNN/model/final_noattn.pth.tar'
    ...
    scheduler = ExponentialLR(optimizer, gamma=0.5) # weight decay的方式（非必要）
